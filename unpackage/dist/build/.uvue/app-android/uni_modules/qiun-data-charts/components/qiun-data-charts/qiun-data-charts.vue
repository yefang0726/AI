...
function deepCloneAssign(origin = {}, ...args): UTSJSONObject {
    for (let i in args) {
...
function formatterAssign(args, formatter): any {
    for (let key in args) {
 ...
 * }, 1000)
 */
function debounce(fn, wait): () => void {
    let timer = false;
    return function () {
        clearTimeout(timer);
...
const __sfc__ = defineComponent({
    ...
    methods: {
        ...
        localdataInit(resdata) {
            ...
            let tmpData = { categories: [], series: [] } as UTSJSONObject;
            let tmpcategories = [];
            let tmpseries = [];
            //拼接categories
            if (this.echarts === true) {
                    ...
                    if (this.inH5 || this.inApp) {
                        ...
                        this.$nextTick(() => {
                            if (this.type2d === true) {
                                ...
                                query
                                    .select('#' + cid)
                                    .fields({ node: true, size: true } as NodeField)
                                    .exec(res => {
                                    if (res[0]) {
        ...
        _tooltipDefault(item, category, index, opts): any {
            if (category) {
                let data = item.data;
        ...
        _showTooltip(e) {
            ...
            if (tc && tc !== undefined && tc != null) {
                ...
                cfu.instance[cid].showToolTip(e, {
                    ...
                    formatter: (item, category, index, opts): any => {
        ...
        _touchMove(e) {
            ...
            if (duration < Math.floor(1000 / touchMoveLimit))
                return; //每秒60帧
